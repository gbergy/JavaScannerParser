...
public class ConcreteSyntax {
	...
	public Program program() {
		...
		Program x = new Program();  //NEW CODE ADDED IN CLASS***
		match("void");  //rule # 3b
		match("main");
		match("(");
		match(")");
		match("{");
		x.decpart = declarations(); //rule #3a
		x.body = statements();
		match("}");   //END NEW CODE ADDED IN CLASS***
	    //return x;     //rule #6   ***Causes error
		
		String[] header = { "void","main", "(", ")" };  //VOID ADDED IN CLASS AS A BUG****
	...
	private Assignment assignment() {
		...
		if (token.getType().equals("Identifier")) {
			// TO BE COMPLETED
			a.target = new Variable();  //added at class
			a.target.id = token.getValue(); //added at class
			match("=");
			a.source= expression();
			match(";");
		} else
